...
const FEATURES = [
  ...
  { icon: Calculator, title: '급여 계산', desc: '근무 데이터 기반 정확한 급여 자동 산출, 오류 제로.' },
  { icon: FileCheck, title: '전자결재', desc: '결재 라인 설정으로 휴가/배차 변경을 빠르게 처리합니다.' },
];
...
const STATS = [
  ...
  { value: '0건', label: '급여 오류', icon: Shield },
];
...
export default function LandingPage() {
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await contactApi.submit({ name: contactName, phone: contactPhone });
      setContactName('');
      setContactPhone('');
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950 text-gray-900 dark:text-white font-sans overflow-x-hidden">
      ...
      {/*  Navigation                                                   */}
      {/* ============================================================ */}
      <nav className="fixed w-full z-50 bg-white/80 dark:bg-gray-950/80 backdrop-blur-md border-b border-gray-200 dark:border-white/10">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => scrollTo('hero')}>
            <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center">
              <span className="font-bold text-white text-lg">B</span>
            </div>
            <span className="text-xl font-bold tracking-tight">Busync</span>
          </div>

          <div className="hidden md:flex items-center gap-6 text-sm font-medium text-gray-600 dark:text-gray-300">
            <button onClick={() => scrollTo('features')} className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">기능</button>
            <button onClick={() => scrollTo('stats')} className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">성과</button>
            <button onClick={() => scrollTo('testimonials')} className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">후기</button>
            <button onClick={() => scrollTo('contact')} className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">문의</button>
          </div>

          <div className="flex gap-3">
            <button
              onClick={() => navigate('/login')}
              className="hidden sm:block text-sm font-medium px-4 py-2 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/10 transition-colors"
            >
              로그인
            </button>
            <button
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid md:grid-cols-4 gap-8 mb-10">
            ...
            <div className="md:col-span-1">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-bold text-lg">Busync</span>
              </div>
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm text-gray-500 dark:text-gray-400">
                <li><button onClick={() => scrollTo('features')} className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">기능 소개</button></li>
                <li><button onClick={() => scrollTo('stats')} className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">도입 효과</button></li>
                <li><button onClick={() => scrollTo('testimonials')} className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">고객 후기</button></li>
              </ul>
            </div>
            <div>
              <h4 className="font-semibold text-sm mb-3">지원</h4>
              <ul className="space-y-2 text-sm text-gray-500 dark:text-gray-400">
                <li><button onClick={() => scrollTo('contact')} className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">도입 문의</button></li>
                <li><button onClick={() => navigate('/login')} className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">로그인</button></li>
              </ul>
            </div>
            <div>
          ...
          <div className="border-t border-gray-200 dark:border-white/10 pt-6 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex gap-6 text-sm text-gray-400">
              <span className="hover:text-gray-600 dark:hover:text-gray-200 cursor-pointer transition-colors">이용약관</span>
              <span className="hover:text-gray-600 dark:hover:text-gray-200 cursor-pointer transition-colors">개인정보처리방침</span>
            </div>
          </div>
        </div>
      </footer>
