  ...
  const saveManualStep = async (s: 'manual-1' | 'manual-2' | 'manual-3') => {
    ...
    try {
      if (s === 'manual-1') {
        ...
        const filled = drivers.filter(d => d.name && d.phone && d.employeeId);
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-gray-900 text-lg">Busync</span>
          </div>
          <span className="text-sm text-gray-500">초기 설정</span>
        </div>
      </header>
            ...
            {step === 'choose' && (
              <div key="choose" className="animate-fade-in-up">
                ...
                >
                  ...
                  <p className="text-lg font-semibold text-gray-800 mb-1">엑셀 파일 올리기</p>
                  <p className="text-gray-500 text-sm mb-5">여기를 누르거나 파일을 여기로 끌어다 놓으세요</p>
                  ...
                  <p className="text-xs text-gray-400 mt-4">
                    ...
                    배차표, 급여대장, 기사 명단 등 어떤 형식이든 괜찮습니다
                  </p>
                      ...
                      <div className="mt-3 space-y-2 max-h-72 overflow-y-auto pr-1">
                        {imported.drivers.map((d, i) => (
                          <div key={i} className="bg-white rounded-lg p-3 border border-gray-200 grid grid-cols-2 gap-2">
                            ...
                            <EditInput value={d.employeeId} placeholder="사원번호"
                              onChange={v => setImported(p => p ? { ...p, drivers: p.drivers.map((x, idx) => idx === i ? { ...x, employeeId: v } : x) } : p)} />
                            <div className="col-span-2 flex gap-2">
                              <EditInput value={d.phone} placeholder="전화번호 (없으면 공백)"
            ...
            {step === 'manual-3' && (
              <div key="m3" className="animate-fade-in-up">
                ...
                <ManualStepHeader icon={<Users size={20} className="text-green-600" />}
                  title="기사 등록"
                  subtitle="기사님의 초기 비밀번호는 사원번호로 자동 설정됩니다. 기사님이 앱에서 직접 바꿀 수 있습니다."
                  step={3} total={3} />
                <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                  <div className="space-y-4">
                    {drivers.map((d, i) => (
                      <div key={i} className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <div>
                            <label className="block text-xs font-medium text-gray-500 mb-1">사원번호 *</label>
                            <input value={d.employeeId} onChange={e => updateDriver(i, 'employeeId', e.target.value)} placeholder="DRV001"
                              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500" />
                          </div>
                          <div>
                  ...
                  <ManualActions saving={saving} isLast={true} onSkip={() => setStep('done')} onNext={() => saveManualStep('manual-3')} />
                </div>
              </div>
            )}
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <NextStepItem
                      ...
                      desc="기사님께 앱 설치 후 사원번호로 로그인하라고 안내해 주세요. 초기 비밀번호 = 사원번호입니다."
                    />
                    <NextStepItem
                      step={3}
                      icon={<MessageSquare size={20} className="text-purple-600" />}
                      title="회사 규칙 AI에게 알려주기"
                      desc="AI 챗봇에서 '연속 6일 이상 근무 금지', '특정 기사 같은 노선 배정 금지' 등 규칙을 입력하세요."
                      action="AI 챗봇 열기"
