...
import { sendBulkPushNotifications, sendPushNotification } from '../../services/notificationService';
...
interface ScoredCandidate {
  ...
  score: number;
  breakdown: { ticketIncentive: number; restBonus: number; familiarityBonus: number };
  signals: {
    goldenTicketBalance: number;
...
async function scoreCandidatesInternal(args: ScoreCandidatesArgs): Promise<ScoredCandidate[]> {
  ...
  // 골든티켓 잔액 + 최근 7일 근무 + 노선 친숙도 — 3개 쿼리 병렬
  const [tickets, recentSlots, familiarAssignments] = await Promise.all([
    prisma.goldenTicket.groupBy({
      by: ['driverId'],
      where: {
        companyId,
        driverId: { in: validIds },
        isUsed: false,
        expiresAt: { gt: virtualNow },
      },
      _count: { id: true },
    }),
  ...
  const ticketMap = new Map(tickets.map((t) => [t.driverId, t._count.id]));
  ...
  const scored: ScoredCandidate[] = drivers.map((d) => {
    const ticketCount = ticketMap.get(d.id) ?? 0;
    const recentWorkDays = fatigueMap.get(d.id) ?? 0;
    const isFamiliar = familiarSet.has(d.id);

    const ticketIncentive = Math.max(0, 40 - ticketCount * 8);
    ...
    return {
      ...
      score: ticketIncentive + restBonus + familiarityBonus,
      breakdown: { ticketIncentive, restBonus, familiarityBonus },
      signals: {
        goldenTicketBalance: ticketCount,
        recentWorkDays,
...
const sendTargetedPush: AgentTool<SendTargetedPushInput, unknown> = {
  ...
  handler: async (input, ctx: ToolContext) => {
    ...
    await sendBulkPushNotifications(
      ...
      input.body,
      'EMERGENCY_SLOT',
      { dropId: input.dropId, agentSession: ctx.sessionId }
    );
...
const recordAcceptance: AgentTool<RecordAcceptanceInput, unknown> = {
  ...
  description:
    ...
    'ScheduleSlot 의 driver 도 새 기사로 변경하고, awardGoldenTicket=true 면 ' +
    '골든티켓 1장을 60일 만료로 발급합니다. 트랜잭션으로 원자 처리됩니다.',
  inputSchema: {
    type: 'object',
    properties: {
      ...
      awardGoldenTicket: {
        type: 'boolean',
        description: '골든티켓 발급 여부 (기본 true)',
      },
    },
  ...
  simulationStub: (input) => ({
    ...
    awardedGoldenTicket: input.awardGoldenTicket ?? true,
  }),
  handler: async (input, ctx: ToolContext) => {
    const award = input.awardGoldenTicket ?? true;
    ...
    const expiresAt = new Date(ctx.virtualNow.getTime() + 60 * 24 * 3600 * 1000);
    ...
    return {
      ...
      goldenTicketAwarded: award,
      goldenTicketExpiresAt: award ? expiresAt.toISOString() : null,
    };
  },
};
...
const escalateToAdmin: AgentTool<EscalateToAdminInput, unknown> = {
  ...
  handler: async (input, ctx: ToolContext) => {
    ...
    const routeNo = drop.slot.route.routeNumber;
    ...
    const body = requirePhone
      ? `${dateStr} ${routeNo}번 ${input.reason}. 즉시 전화로 기사에게 연락하세요.`
      : `${dateStr} ${routeNo}번 ${input.reason}. 자동 대응 진행 중, 모니터링 권장.`;
...
const requestSwap: AgentTool<RequestSwapInput, unknown> = {
  ...
  handler: async (input, ctx: ToolContext) => {
    ...
    const routeNo = drop.slot.route.routeNumber;
    const dateStr = drop.slot.date.toISOString().slice(0, 10);
