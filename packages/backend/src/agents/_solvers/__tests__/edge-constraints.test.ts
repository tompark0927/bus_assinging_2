...
// ─────────────────────────────────────────────
// SUITE 13: INV-L — Cross-route move safety (SP4-3)
//
// Invariants:
//   INV-L1  canCrossRoute=false drivers NEVER appear on a foreign route via
//           Phase C FILL / REASSIGN / SWAP moves.
//   INV-L2  Every slot with familiarity==='CROSS_ROUTE' has driver.canCrossRoute===true
//           (the EMERGENCY Phase B tier is excluded from this assertion per design).
//   INV-L3  2-route scenario: route A has unfilled slot, route B has idle
//           canCrossRoute=true spare → spare fills A's slot (unfilled drops 0).
//   INV-L4  canCrossRoute=false driver is NEVER placed cross-route.
// ─────────────────────────────────────────────

/**
 * Assert INV-L2: every CROSS_ROUTE slot that came from Phase C (not Phase B EMERGENCY)
 * must have driver.canCrossRoute===true.
 *
 * Because Phase B EMERGENCY CAN place canCrossRoute=false drivers cross-route
 * (documented design), we cannot blanket-assert all CROSS_ROUTE slots.  However,
 * for inputs that are constructed so no EMERGENCY is needed (each route has
 * at least one always-available non-expired driver), all CROSS_ROUTE slots must
 * be from canCrossRoute===true drivers.
 *
 * Returns violations: slots where familiarity==='CROSS_ROUTE' and
 * driver.canCrossRoute !== true.
 */
function checkInvL2_CrossRouteDriverEligibility(
  slots: AssignedSlot[],
  drivers: SolverDriver[],
): Array<{ slot: AssignedSlot; reason: string }> {
  const driverMap = new Map(drivers.map((d) => [d.id, d]));
  return slots
    .filter((s) => s.familiarity === 'CROSS_ROUTE')
    .map((s) => {
      const d = driverMap.get(s.driverId);
      if (!d) return null;
      if (d.canCrossRoute !== true) {
        return {
          slot: s,
          reason: `CROSS_ROUTE slot assigned to driver ${d.id} (canCrossRoute=${d.canCrossRoute}) who is not eligible`,
        };
      }
      return null;
    })
    .filter((v): v is { slot: AssignedSlot; reason: string } => v !== null);
}

describe('INV-L: Cross-route move safety (SP4-3)', () => {

  /**
   * INV-L3 (TDD): 2-route scenario — route A unfilled slot, route B idle canCrossRoute spare.
   *
   * BEFORE the fix (same-route-only FILL): route A's slot stays unfilled because
   * the only available driver (D5 on route B) is cross-route.
   * AFTER the fix: D5 fills A's slot via Phase C-2 FILL → unfilled drops to 0.
   *
   * Setup:
   *   Route 10: bus 1, crew [D1, D2]. Both drivers have ALL month days as approved-off
   *             except May 1–3, ensuring bus 1 has unfilled slots after May 3.
   *   Route 20: bus 2, crew [D3, D4], plus idle spare D5 (canCrossRoute=true, homeRouteId=20).
   *             D3, D4 have approved-off for whole month so they NEVER work.
   *             D5 is always available (no day-offs) and canCrossRoute=true.
   *
   * With same-route-only FILL, route 10's unfilled slots cannot be filled (D1,D2 are off).
   * With cross-route FILL enabled, D5 fills them.
   *
   * canCrossRoute=false assertion: D3 and D4 (canCrossRoute=false) must NEVER appear
   * on route 10 in the output.
   */
  test('INV-L3: cross-route FILL fills otherwise-empty slots (TDD regression)', () => {
    const allDays = monthDates(YEAR, MONTH);
    // D1, D2 only available May 1–3; rest of month is approved-off
    const d1d2Off = allDays.filter((d) => {
      const day = parseInt(d.slice(8), 10);
      return day > 3;
    });
    // D3, D4 entirely off (never available)
    const allOff = allDays;

    const input: SolverInput = {
      year: YEAR, month: MONTH,
      drivers: [
        makeDriver(1, { homeBusId: 1, homeRouteId: 10, canCrossRoute: false, approvedDayOffs: d1d2Off }),
        makeDriver(2, { homeBusId: 1, homeRouteId: 10, canCrossRoute: false, approvedDayOffs: d1d2Off }),
        makeDriver(3, { homeBusId: 2, homeRouteId: 20, canCrossRoute: false, approvedDayOffs: allOff }),
        makeDriver(4, { homeBusId: 2, homeRouteId: 20, canCrossRoute: false, approvedDayOffs: allOff }),
        // Idle spare on route 20 with canCrossRoute=true — the key actor
        makeDriver(5, { homeRouteId: 20, canCrossRoute: true }),
      ],
      buses: [makeBus(1, 10), makeBus(2, 20)],
      crews: [
        makeCrew('C1', [1, 2], 1, 10),
    ...
    assertHardInvariants('INV-L3', input, output);

    // After Day 3, D1 & D2 are off. D5 (canCrossRoute=true) should fill route 10 slots.
    // Expect D5 to appear on route 10 for slots after May 3.
    const d5OnRoute10 = output.slots.filter((s) => s.driverId === 5 && s.routeId === 10);
    expect(d5OnRoute10.length).toBeGreaterThan(0); // cross-route fill happened

    // The slots D5 filled on route 10 should have familiarity==='CROSS_ROUTE'
    for (const s of d5OnRoute10) {
      expect(s.familiarity).toBe('CROSS_ROUTE');
    }

    // INV-L4: canCrossRoute=false drivers (D1,D2 homeRouteId=10; D3,D4 homeRouteId=20)
    // must NEVER appear on a route != their homeRouteId via Phase C moves.
    // D1,D2 must not appear on route 20; D3,D4 must not appear on route 10.
    const d1d2OnRoute20 = output.slots.filter(
      (s) => (s.driverId === 1 || s.driverId === 2) && s.routeId === 20,
    );
    expect(d1d2OnRoute20).toHaveLength(0);

    const d3d4OnRoute10 = output.slots.filter(
      (s) => (s.driverId === 3 || s.driverId === 4) && s.routeId === 10,
    );
    expect(d3d4OnRoute10).toHaveLength(0);
  });

  /**
   * INV-L2 regression: every CROSS_ROUTE output slot (in a scenario where
   * Phase B EMERGENCY cannot fire) must have driver.canCrossRoute===true.
   *
   * Design to prevent EMERGENCY:
   *   - Single route (route 10), so cross-route EMERGENCY never applies.
   *   - Spare D9 has canCrossRoute=true and homeRouteId=10 (same route), so D9
   *     is a SAME_ROUTE candidate for route 10 — EMERGENCY never fires.
   *   - D9 appears in output with familiarity SAME_ROUTE (not CROSS_ROUTE) because
   *     homeRouteId matches the single route in this scenario.
   *
   * This test asserts that in a multi-route scenario, the FILL move does NOT
   * produce CROSS_ROUTE slots for ineligible drivers. We verify this by checking
   * the output of the INV-L3 scenario (which has both routes) using the
   * checkInvL2 function — after the SP4-3 fix, only D5 (canCrossRoute=true)
   * should appear with familiarity=CROSS_ROUTE on route 10.
   */
  test('INV-L2: CROSS_ROUTE slots from Phase C FILL have driver.canCrossRoute===true', () => {
    // Re-use the INV-L3 scenario: route 10 (bus 1, crew [D1,D2]) + route 20 (bus 2, crew [D3,D4], spare D5).
    // D1,D2 mostly off after May 3. D5 (canCrossRoute=true) fills route 10 cross-route.
    // D3,D4 (canCrossRoute=false) must NOT fill route 10.
    const allDays = monthDates(YEAR, MONTH);
    const d1d2Off = allDays.filter((d) => {
      const day = parseInt(d.slice(8), 10);
      return day > 3;
    ...
    const output = solveMonthlyGrid(input);
    assertHardInvariants('INV-L2-fill', input, output);

    // Every CROSS_ROUTE slot must be assigned to a driver with canCrossRoute===true.
    // D3 and D4 (canCrossRoute=false) must never appear on route 10 with CROSS_ROUTE familiarity.
    const crossRouteSlots = output.slots.filter((s) => s.familiarity === 'CROSS_ROUTE');
    const driverMap = new Map(input.drivers.map((d) => [d.id, d]));
    for (const s of crossRouteSlots) {
      const d = driverMap.get(s.driverId);
      expect(d?.canCrossRoute).toBe(true);
    }
    // Specifically D3 and D4 must not appear on route 10 at all
    expect(output.slots.filter((s) => (s.driverId === 3 || s.driverId === 4) && s.routeId === 10)).toHaveLength(0);
  });

  /**
   * INV-L1 (Phase C isolation): canCrossRoute=false drivers must not appear
   * on a foreign route due to Phase C FILL, REASSIGN, or SWAP moves.
   *
   * We use localSearchIterations=0 as a control (Phase B only) and compare
   * with a full run. The cross-route violation count must not INCREASE due to
   * Phase C moves; the test checks this by comparing both outputs.
   *
   * Note: Phase B EMERGENCY CAN place canCrossRoute=false drivers cross-route
   * (documented design). This test only checks that Phase C does NOT add new ones.
   *
   * With a 2-route, 2-bus setup, EMERGENCY may fire — so we track the Phase B
   * baseline cross-route violation count and assert Phase C doesn't add more.
   */
  test('INV-L1: Phase C does not add canCrossRoute=false drivers to foreign routes', () => {
    const drivers: SolverDriver[] = [
      makeDriver(1, { homeBusId: 1, homeRouteId: 10, canCrossRoute: false }),
      makeDriver(2, { homeBusId: 1, homeRouteId: 10, canCrossRoute: false }),
      makeDriver(3, { homeBusId: 2, homeRouteId: 20, canCrossRoute: false }),
      makeDriver(4, { homeBusId: 2, homeRouteId: 20, canCrossRoute: false }),
      makeDriver(5, { homeRouteId: 20, canCrossRoute: true }), // only cross-route eligible
    ];
    const baseInput: SolverInput = {
      ...
      policy: CITY,
      localSearchIterations: 0, // Phase B only — establishes EMERGENCY baseline
      randomSeed: 11223,
    };
    const fullInput: SolverInput = { ...baseInput, localSearchIterations: 15000 };

    const baseOutput = solveMonthlyGrid(baseInput);
    const fullOutput = solveMonthlyGrid(fullInput);

    assertHardInvariants('INV-L1-base', baseInput, baseOutput);
    assertHardInvariants('INV-L1-full', fullInput, fullOutput);

    const driverMap = new Map(drivers.map((d) => [d.id, d]));
    const countFalseCrossRoute = (slots: AssignedSlot[]) =>
      slots.filter((s) => {
        const d = driverMap.get(s.driverId);
        return d && d.canCrossRoute !== true && d.homeRouteId !== undefined && d.homeRouteId !== s.routeId;
      }).length;

    const baseCount = countFalseCrossRoute(baseOutput.slots);
    const fullCount = countFalseCrossRoute(fullOutput.slots);

    // Phase C must NOT increase the number of canCrossRoute=false cross-route placements
    // (it may reduce them if it replaces EMERGENCY placements, but must not add new ones)
    expect(fullCount).toBeLessThanOrEqual(baseCount);
  });

  /**
   * INV-L4 (SWAP isolation): canCrossRoute=false drivers must not appear
   * on a foreign route due to cross-route SWAP moves. We verify by comparing
   * Phase B baseline (0 iterations) to full run (20000 iterations).
   * SWAP must not move canCrossRoute=false drivers cross-route.
   */
  test('INV-L4: cross-route SWAP does not move canCrossRoute=false drivers to foreign routes', () => {
    const drivers: SolverDriver[] = [
      // Route 10: D1 (false) + D7 (true, canCrossRoute=true)
      makeDriver(1, { homeBusId: 1, homeRouteId: 10, canCrossRoute: false }),
      makeDriver(2, { homeBusId: 1, homeRouteId: 10, canCrossRoute: false }),
      makeDriver(7, { homeBusId: 2, homeRouteId: 10, canCrossRoute: true }),
      makeDriver(8, { homeBusId: 2, homeRouteId: 10, canCrossRoute: true }),
      // Route 20: D5 (false) + D9 (true, canCrossRoute=true)
      makeDriver(5, { homeBusId: 3, homeRouteId: 20, canCrossRoute: false }),
      makeDriver(6, { homeBusId: 3, homeRouteId: 20, canCrossRoute: false }),
      makeDriver(9, { homeBusId: 4, homeRouteId: 20, canCrossRoute: true }),
      makeDriver(10, { homeBusId: 4, homeRouteId: 20, canCrossRoute: true }),
    ];
    const baseInput: SolverInput = {
      year: YEAR, month: MONTH,
      drivers,
      buses: [makeBus(1, 10), makeBus(2, 10), makeBus(3, 20), makeBus(4, 20)],
      crews: [
        makeCrew('C1', [1, 2], 1, 10),
        makeCrew('C2', [7, 8], 2, 10),
        makeCrew('C3', [5, 6], 3, 20),
        makeCrew('C4', [9, 10], 4, 20),
      ],
      policy: CITY,
      localSearchIterations: 0, // Phase B only — establishes EMERGENCY baseline
      randomSeed: 55667,
    };
    const fullInput: SolverInput = { ...baseInput, localSearchIterations: 20000 };

    const baseOutput = solveMonthlyGrid(baseInput);
    const fullOutput = solveMonthlyGrid(fullInput);

    assertHardInvariants('INV-L4-base', baseInput, baseOutput);
    assertHardInvariants('INV-L4-full', fullInput, fullOutput);
    ...
    // Phase C must NOT add new canCrossRoute=false cross-route placements vs Phase B baseline.
    // Note: Phase C may REORGANIZE existing EMERGENCY placements (change their date) — this
    // is acceptable as long as the total count doesn't increase.
    expect(fullCount).toBeLessThanOrEqual(baseCount);
  });
