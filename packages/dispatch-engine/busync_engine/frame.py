...
사이클 (성민, 2026-08-31 확정)
    근무 5일(오후) → 휴무 1일 → 근무 5일(오전) → 휴무 2일   = 13일

    1일부터 시작한 짝꿍의 휴무일: 6 · 12 · 13 · 19 · 25 · 26 …

    · 정·부는 **같은 날 함께 근무하고 함께 쉰다**. 시프트만 서로 반대다.
    · 차량마다 하루씩 밀린다 — 1번 차가 6일에 쉬면 2번 차는 7일에 쉰다.
      이 계단 덕분에 하루에 쉬는 차가 14대 중 3.2대로 고르게 퍼지고,
      평일 12대·토 11대·휴일 10대 운행과 맞물린다.
"""
...
#: 첫 근무 블록이 오후인 것은 사장님 확인 사항이다("오후 5일 쭉 일하고").
CYCLE: tuple[tuple[bool, int], ...] = (
    (True, 5),    # 근무 — 정기사 오후 / 부기사 오전
    (False, 1),   # 휴무 1일
    (True, 5),    # 근무 — 정기사 오전 / 부기사 오후
    (False, 2),   # 휴무 2일
)

CYCLE_LEN = sum(n for _w, n in CYCLE)  # 13
